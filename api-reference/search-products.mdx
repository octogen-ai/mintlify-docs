...
description: "Searches products within a single granted catalog using free-text queries, structured facet filters, price ranges, and cursor-based pagination."
...
<ParamField body="catalog" type="string" required>
  Key of the catalog to search. Must be a catalog currently granted to your organization. Retrieve available keys from [`GET /catalogs`](/api-reference/list-catalogs). Cross-catalog search is not supported — issue separate requests for each catalog.
</ParamField>
