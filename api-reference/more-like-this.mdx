...
<ParamField body="source.image.url" type="string">
  HTTPS image URL fetched server-side. Available only for hosts on the deployment allowlist (ask your Octogen contact to enable hosts); redirects are not followed and the same 8 MB cap applies while streaming.
</ParamField>
...
<ParamField body="cursor" type="string">
  Opaque pagination cursor from a previous response's `nextCursor` field. Pass it unmodified with the same request fields to retrieve the next page. Not supported for image sources (`400`): the query is generated per request, so a cursor from one request is not valid against another's.
</ParamField>
...
- **Latency**: image queries are generated per request — expect p50 ≤ 8s and ~11s when relaxation rounds run (product sources stay search-only fast). The response's `resolution` field reports how the source was resolved; caching for repeat images is on the roadmap and will bring them to ~2s.
...
- **Response shape**: image responses return `sourceImage.hash` and `resolution` instead of the resolved-product `source` stanza; `nextCursor` is always `null`.
...
<ResponseField name="resolution" type="string">
  Present for image sources only. How the source was resolved; currently always `"image_query_generation"` (the generation pipeline ran).
</ResponseField>
