...
<ResponseField name="bigQuery" type="object | null" required>
  ...
  `sharedDatasetId` the shared dataset, and `viewId` the queryable view
  (`products_current_v1`). `lastExportedAt` and `lastRowCount` describe the
  most recent daily export, and `readerCount` is the number of your
  registered BigQuery Readers with access.
</ResponseField>
