...
The document includes server URLs, operation IDs (`listCatalogs`, `searchProducts`, `lookupProduct`, `recrawlProducts`), full request and response schemas, and example error bodies. Most ecosystems can generate a typed client from it - for example `openapi-generator`, `openapi-typescript`, or `oapi-codegen`.
...
| `POST` | `/products/recrawl` | `recrawlProducts` | Schedules product URLs or UUIDs for an on-demand crawl. |
