...
API keys are organization-scoped and currently usable only by Catalog Partner organizations. A key can search and browse every active crawled catalog, including catalogs activated later, and can never access merchant catalogs. Rotate or deactivate keys via the partner portal; deactivation takes effect on the next request.
