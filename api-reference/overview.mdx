...
The document includes server URLs, operation IDs such as `searchProducts`, `moreLikeThisProducts`, `lookupProduct`, `refreshProducts`, `startVoyage`, `getVoyage`, and `listVoyages`, full request and response schemas, and example error bodies. Most ecosystems can generate a typed client from it - for example `openapi-generator`, `openapi-typescript`, or `oapi-codegen`.
