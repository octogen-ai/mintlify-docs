...
Three tools are exposed by the MCP server. Each maps 1:1 to an endpoint in the [Platform Catalog API v1](/api-reference/overview), so the argument names and response shapes are the same on either surface.
