...
description: "Connect Codex CLI to Octogen using its dedicated compatibility endpoint, then sign in once to give your agent access to your granted catalogs."
...
- **`search_products(catalog?, query?, limit?, cursor?)`** — search products by keyword across all granted catalogs, or within one catalog when `catalog` is provided.
