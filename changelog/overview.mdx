...
**Platform Catalog API v1 (REST) is generally available.** Backends and batch jobs can now hit the same catalog data over HTTPS with an `octo_live_...` API key. Endpoints include `POST /v1/catalogs`, `POST /v1/products/lookup`, and `POST /v1/products/search`, with cursor-based pagination and facet support.
