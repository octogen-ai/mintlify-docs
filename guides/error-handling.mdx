...
<AccordionGroup>
  ...
  <Accordion title="404 — Not found">
    **SDK exception:** `OctogenNotFoundError`
    ...
    - `"product_not_found"` — No active product in an active crawled catalog matches the URL you passed to `/products/lookup`.

    **Recovery:** Retry with a known active crawled catalog key, or omit the optional `catalog` field for policy-wide search. For product lookup, confirm the URL is a canonical product page URL.
  </Accordion>
