...
- The storefront blocks server-side fetching. Your user's browser is a legitimate session, so pages that no crawler can retrieve still resolve from a capture.
...
The `url` field is optional in the contract, but in this model you always have it — `location.href` — so always send it. It does three jobs:
...
If a capture genuinely arrives without a URL, the response carries a `source_url_missing` warning and only absolute image URLs survive. The `url` is caller-asserted and unverified — it is a parsing hint, not provenance, which is why these results carry `source: "client_html"` rather than `"on_demand"`.
