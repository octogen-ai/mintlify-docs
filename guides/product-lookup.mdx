...
When you call `POST /v1/products/lookup` with a product page URL, Octogen matches the URL against active product records across active crawled catalogs and returns the canonical record from the first matching catalog. The response tells you which catalog matched (`catalogKey`, `catalogDisplayName`) alongside the full product object.
...
<Note>
  If no active product in an active crawled catalog matches the URL, the API returns HTTP `404` with `"product_not_found"` in the error detail. The SDK raises `OctogenNotFoundError`. This is not a retryable error — it means the URL is not indexed in any catalog available to Catalog Partners.
</Note>

Check that the URL is a canonical product page URL (not a search results page or category page) from an active crawled catalog.
